...
{
    bool result = false;
	data->filename = subnode->GetAttribute(wxT("name"),wxT("")).Trim(TRUE).Trim(FALSE);
	...
	while (subsubnode)
	{
        ...
        else if (subsubnode->GetName().Lower() == wxT("verification"))
        {
            ...
            while (subsubsubnode)
            {
                ...
                {
                    if (subsubsubnode->GetAttribute(wxT("type"),wxEmptyString).Lower() == wxT("md5"))
                        data->md5 = this->GetContent(subsubsubnode->GetChildren());
                    else if (subsubsubnode->GetAttribute(wxT("type"),wxEmptyString).Lower() == wxT("sha1"))
...
{
    if (children)
        return wxString(children->GetContent()).Trim(TRUE).Trim(FALSE);
    else
